...
void SetUpFCPCameraApiWithSuffix(id<FlutterBinaryMessenger> binaryMessenger, NSObject<FCPCameraApi> *api, NSString *messageChannelSuffix) {
  ...
  /// 1 means the lens is at the maximum position.
  {
    FlutterBasicMessageChannel *channel =
      [[FlutterBasicMessageChannel alloc]
        initWithName:[NSString stringWithFormat:@"%@%@", @"dev.flutter.pigeon.camera_avfoundation.CameraApi.setLensPosition", messageChannelSuffix]
        binaryMessenger:binaryMessenger
        codec:FCPGetMessagesCodec()];
    if (api) {
      NSCAssert([api respondsToSelector:@selector(setLensPosition:completion:)], @"FCPCameraApi api (%@) doesn't respond to @selector(setLensPosition:completion:)", api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray<id> *args = message;
        double arg_position = [GetNullableObjectAtIndex(args, 0) doubleValue];
        [api setLensPosition:arg_position completion:^(FlutterError *_Nullable error) {
          callback(wrapResult(nil, error));
        }];
      }];
    } else {
